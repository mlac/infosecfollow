...
for name, alpha in JOBS:
    ...
    for j, P in enumerate(PERIODS):
        obs, K = climb(C, P, PERM[alpha], rng, R); nrun += 1
        nl = np.array([climb(b, P, PERM[alpha], rng, R)[0] for b in bank]); nrun += NNULL
        ...
        print(f"  {name}/{alpha} p={P:3d} obs={obs:.4f} null={mu:.4f}+-{sd:.4f} "
    ...
    out[f"{name}_{alpha}"] = dict(rows=rows, family=fam)
    print(f"### {name}/{alpha} FAM obs_maxz={obsmax:+.2f} @p={fam['obs_argmax']} "
          f"nullmaxz mean={mz.mean():.2f} max={mz.max():.2f} fam_p={fam['fam_p']:.3f}", flush=True)
    json.dump(out, open('results/lp_hill_raw.json', 'w'))
